...
def plot_upper_dash():
    ...
    total_today_plot = alt.Chart(pd.DataFrame({'vaccines_today': [vaccines_today]}).applymap('{:,.0f}'.format)).mark_text(
        size=summary_text_size).encode(text='vaccines_today:N').properties(width=summary_width, height=summary_height,
    ...
    us_total_plot = alt.Chart(summary_usa).mark_text(size=summary_text_size).encode(
        text='total_vaccinations_raw:N').properties(width=summary_width, height=summary_height,
